	...
	PickerHandler.prototype._relocate = function(){
		...
		if ($input != null && $picker.data('isInline') === false) { // Float mode
			...
			if($(".datepicker_calendar", $picker).width() !== 0 && $(".datepicker_timelist", $picker).width() !== 0){
				$picker.parent().width($(".datepicker_calendar", $picker).width() + $(".datepicker_timelist", $picker).width() + 16);
			}
			if(parseInt($(window.parent.document).height()) <=  ($input.offset().top - $(window.parent.document).find(".main-right").scrollTop() + input_outer_height + picker_outer_height +70) ){	
				//在iframe里获取的当前视窗高度以及scrollTop值，须通过$(window.parent.document)来获取，否则.height()不准确，.scrollTop()始终为0。70为顶部黑色块高度
	...
	**/
	...
	var draw = function($picker, option, year, month, day, hour, min) {
		...
		if (isTodayButton) {
			...
			//增加关闭按钮
			var $close_btn = $('<a class="icon-close" style="float:right; margin: -5px -8px 0 0;"><img src="images/icon-close.png"></a>');
			/*
		...
		//增加清除按钮
		//$header.append('<span class="clear-value-btn">清除</span>');
	 ...
	 $.fn.appendDtpicker = function(config) {
		...
		return this.each(function(i) {
			...
			if(options.inline === true){
				...
				//清除日期
				/*$(document).on('click', '.clear-value-btn', function(){
					if($(input).val() !== ''){
						$(input).val('');
					}
				});*/
